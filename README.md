...
**Around the sessions**
- Git panel per session: status, diff, log, commit, push, PR creation, and background
  CI polling of the PR's checks with desktop notifications
...
| Tool | Version | Notes |
|---|---|---|
| JDK | 25 (Temurin) | `./mvnw -v` must report Java 25 |
...
If you use only Codex, set **Settings → Sessions → Default provider** to `codex`.
The `claude` CLI is then not needed — with one caveat: backend-initiated helper turns
that need MCP tools (Linear ticket import, memory-tool flows) can't answer approval
prompts on a Codex system session and will time out; plain helper tasks (auto-titles,
commit-message drafts) work fine.

## Feature setup guides

Each opt-in feature is configured in the Settings dialog (persisted, no restart) or by
env var; deep-dives live in the phase docs:
...
| Orchestration (child sessions) | none — agents get a `spawn_child_session` tool | [`docs/plan/phase-7-ux-and-orchestration.md`](docs/plan/phase-7-ux-and-orchestration.md) |
